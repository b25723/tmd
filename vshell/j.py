...
def form(cmd):
    ...
    for i in range(0,len(cmd)):
            #pos=list(itertools.permutations(range(0,len(cmd[i])),len(cmd[i])))+[''.join(inj[i])]
            #pos=list(itertools.permutations(range(0,len(cmd[i])),len(cmd[i])))
            #for j in cmd[i]:
            #for j in range(0,len(cmd[i])):
            #print(list(itertools.permutations(range(0,len(cmd[i])),len(cmd[i])))+[''.join(inj[i])])
            #print(list(itertools.permutations(list(itertools.permutations(range(0,len(cmd[i])),len(cmd[i])))+[''.join(inj[i])])))
