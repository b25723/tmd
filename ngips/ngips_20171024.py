...
def copy_ips(counter=3):
    ...
    for i in range(1,counter+1):
        s=requests.post(URL+IPS+'/1/duplicate',cookies={'sid':req.json()['sid']},verify=False)
        print s.status_code
...
def add_schds(counter=1):
    for i in range(1,counter+1):
        ...
        s=requests.post(URL+'/rest/object/schds',json=payload,cookies={'sid':req.json()['sid']},verify=False)
        print s.status_code
        print s.content
        #print payload
